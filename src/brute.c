...
void *worker(void *arg) {
    ...
    for(uint64_t i = data->start; i < data->end && !atomic_load_explicit(&found, memory_order_relaxed); i++) { // atomic_load retrives safelly value
        index_to_password(i, guess, data->lenght);
